...
from utils import fork_simulate
import argparse
import configparser
import asyncio
...
async def main():
    ...
    parser.add_argument('--fork_length', type=int,
                        help='分叉长度', default=config.getint('input_args', 'fork_length'))
    ...
    for malicious_ratio in np.arange(args.malicious_ratio_min, args.malicious_ratio_max+args.malicious_ratio_perinterval, args.malicious_ratio_perinterval):
        ...
        blockchain_length, malicious_forks, average_selfish_mined =await fork_simulate(
            args.num_nodes, malicious_ratio, args.rounds, args.fork_length,difficulty,args.attempts)
        
        results.append([malicious_ratio, blockchain_length, malicious_forks, average_selfish_mined])
        

    # print the experiment result in the form of chart
    headers = ["Malicious Ratio", "Final Blockchain Length",
               "Malicious Forks", "Average Selfish Mined Blocks"]
    print("\nResults:")
